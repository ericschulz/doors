...
table(dexp1$gender)
##age in months
mean(na.omit(dexp1$age))
sd(na.omit(dexp1$age))
```
...
```{r}
ttestBF(subset(dexp1, condition=="1-animal")$ndoors, 
        subset(dexp1, condition=="8-animals")$ndoors,
        nullInterval=c(-Inf,0))

cohensD(subset(dexp1, condition=="1-animal")$ndoors, 
        subset(dexp1, condition=="8-animals")$ndoors)

t.test(subset(dexp1, condition=="1-animal")$ndoors, 
       subset(dexp1, condition=="8-animals")$ndoors,
       nullInterval=c(-Inf,0), 
       var.equal = TRUE)
...
```{r}
ttestBF(subset(dexp1, condition=="1-animal")$latency, 
        subset(dexp1, condition=="8-animals")$latency,
        nullInterval=c(-Inf,0))

cohensD(subset(dexp1, condition=="1-animal")$latency, 
        subset(dexp1, condition=="8-animals")$latency)

t.test(subset(dexp1, condition=="1-animal")$latency, 
       subset(dexp1, condition=="8-animals")$latency,
       nullInterval=c(-Inf,0), 
       var.equal = TRUE)
...
```{r}
chisq.test(table(dexp1$completion==1, dexp1$condition=="8-animals"))

contingencyTableBF(table(dexp1$completion==1, dexp1$condition=="8-animals"),
...
```{r}
m<-MCMCregress(ndoors~age+condition, data=dexp1)
summary(m)
```
